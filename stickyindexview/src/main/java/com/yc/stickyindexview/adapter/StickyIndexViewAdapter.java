...
import com.yc.stickyindexview.IndexValueBus;
...
/**
 * {@link StickyIndexView}数据适配器
 *
 * @author by KABUQINUOFU on 2018/5/11.
 */
public abstract class StickyIndexViewAdapter<T extends BaseEntity> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
     ...
     * 列表中实际展示时用到的实体数据
     */
    private List<StickyIndexEntity<T>> mList;
    /**
     * 索引值在列表中位置的映射关系
     */
    ...
    private List<StickyIndexEntity<T>> mIndexHeaderList = new ArrayList<>();
    private Map<String, Integer> mIndexHeaderValuePositionMap = new HashMap<>();
    private SparseArray<IndexHeaderOrFooterAdapter<T>> mIndexHeaderAdapters = new SparseArray<>(0);

    private List<String> mIndexFooterValueList = new ArrayList<>();
    private List<StickyIndexEntity<T>> mIndexFooterList = new ArrayList<>();
    private Map<String, Integer> mIndexFooterValuePositionMap = new HashMap<>();
    private SparseArray<IndexHeaderOrFooterAdapter<T>> mIndexFooterAdapters = new SparseArray<>(0);

    public StickyIndexViewAdapter(List<T> originalList) {
    ...
    private void transferOriginalData() {
        ConvertUtil.ConvertResult convertResult = ConvertUtil.transfer(mOriginalList);
     ...
     * @return
     */
    public final List<StickyIndexEntity<T>> getData() {
        return mList;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == ItemType.ITEM_TYPE_INDEX) {//StickyIndex索引类型视图
            viewHolder = onCreateIndexViewHolder(parent);
        } else if (viewType == ItemType.ITEM_TYPE_CONTENT) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        } else if (viewType == ItemType.ITEM_TYPE_CONTENT) {//StickyIndex索引类型视图正常Item内容视图
    ...
    private void addListener(final RecyclerView.ViewHolder viewHolder, final int viewType) {
        ...
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                StickyIndexEntity<T> entity = getItem(position);
                ...
                } else if (mIndexHeaderAdapters.indexOfKey(viewType) >= 0) {
                    IndexHeaderOrFooterAdapter<T> adapter = mIndexHeaderAdapters.get(viewType);
                ...
                } else if (mIndexFooterAdapters.indexOfKey(viewType) >= 0) {
                    IndexHeaderOrFooterAdapter<T> adapter = mIndexFooterAdapters.get(viewType);
                    if (adapter.getOnItemClickListener() != null) {
                        adapter.getOnItemClickListener().onItemClick(viewHolder.itemView, position, entity.getOriginalData());
                    }
                }
    ...
    public int getItemCount() {
        return (mList == null || mIndexHeaderList == null || mIndexFooterList == null) ? 0 : mList.size() + mIndexHeaderList.size() + mIndexFooterList.size();
    }

    public final StickyIndexEntity<T> getItem(int position) {
     ...
     * 设置Header自定义索引数据项适配器
     **/
    public void addIndexHeaderAdapter(IndexHeaderOrFooterAdapter<T> indexHeaderAdapter) {
     ...
     * 设置Footer自定义索引数据项适配器
     **/
    public void addIndexFooterAdapter(IndexHeaderOrFooterAdapter<T> indexFooterAdapter) {
     ...
     * @param adapter
     */
    public final void removeHeader(IndexHeaderOrFooterAdapter adapter) {
     ...
     * @param adapter
     */
    public final void removeFooter(IndexHeaderOrFooterAdapter adapter) {
     ...
     * 删除所有Footer
     */
    private void removeAllFooter(boolean isRefresh) {
