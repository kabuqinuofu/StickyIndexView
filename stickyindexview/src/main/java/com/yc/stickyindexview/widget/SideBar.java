package com.yc.stickyindexview.widget;
...
import com.yc.stickyindexview.R;
...
public class SideBar extends View implements View.OnTouchListener {
    ...
    //默认选中时的文字颜色
    private static final int DEFAULT_FOCUS_TEXT_COLOR = Color.WHITE;
    //默认选中时的背景色
    ...
    //默认bar的背景色
    private static final int DEFAULT_BAR_BACKGROUND = Color.TRANSPARENT;
    ...
    private RectF mRectF = new RectF(); //RectF对象
    ...
    public void initAttrs(TypedArray typedArray) {
        ...
        mTextColor = typedArray.getColor(R.styleable.StickyIndexView_sideBarTextColor, DEFAULT_TEXT_COLOR);
        mFocusTextColor = typedArray.getColor(R.styleable.StickyIndexView_sideBarFocusTextColor, DEFAULT_FOCUS_TEXT_COLOR);
        mFocusBackgroundColor = typedArray.getColor(R.styleable.StickyIndexView_sideBarFocusBackgroundColor, DEFAULT_FOCUS_BACKGROUND_COLOR);
        mTextSize = typedArray.getDimensionPixelSize(R.styleable.StickyIndexView_sideBarTextSize, 0);
        mTextSpace = typedArray.getDimensionPixelSize(R.styleable.StickyIndexView_sideBarTextSpace, 0);
        mBackground = typedArray.getColor(R.styleable.StickyIndexView_sideBarBackgroundColor, DEFAULT_BAR_BACKGROUND);
...
//                canvas.drawCircle(getWidth() / 2, mItemHeight / 2 + mItemHeight * i, radius, mFocusTextBgPaint);
                //TODO 由原有的画圆逻辑修改为画圆角矩形逻辑
                /*********************************绘制圆角矩形开始******************************/

//                RectF mRectF = new RectF();                       //RectF对象
                mRectF.left = 5;                                  //设置在父容器中距离左边的距离
                mRectF.top = mItemHeight * i;                    //设置在父容器中距离上边的距离
                mRectF.right = getWidth() - 5;                   //设置在父容器中距离右边的距离
                mRectF.bottom = mItemHeight + mItemHeight * i;   //设置在父容器中距离下边的距离
                canvas.drawRoundRect(mRectF, 10, 10, mFocusTextBgPaint);        //绘制圆角矩形
